function [interp_d] = interp_data_spectral(L,n_grid, int_r, scaling_factor, r, p, farfield_val)
...
% int_r = the max interaction dist. in terms of # Angstroms %this should
%       be renamed. It is the domain length of the original.
...
dx = L / n_grid;
grid_1D = linspace( (-L/2), (L/2 -dx), (scaled_n_grid .* 2));
...
filter = ( R <= int_r );
interp_d(filter) = interp1( r, p, R(filter), "linear", farfield_val);
